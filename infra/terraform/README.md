# Terraform — Scribe on AWS (Amplify + S3)
...
```bash
aws amplify start-job --app-id <APP_ID> --branch-name cloud --job-type RELEASE --region ap-southeast-2
```
...
```bash
cd ../..
S3_BUCKET=<bucket> AWS_REGION=ap-southeast-2 BOOKS_DIR=../books npm run seed:s3
```
